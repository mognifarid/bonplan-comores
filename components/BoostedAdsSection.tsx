    ...
    <section className="animate-fade-in">
      <div className="bg-gradient-to-r from-accent/10 via-primary/5 to-destructive/10 rounded-2xl p-6 border border-border/50">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-3 mb-6 h-12 bg-card/80 backdrop-blur-sm">
            <TabsTrigger 
              value="vedette" 
              className="gap-2 data-[state=active]:bg-gradient-premium data-[state=active]:text-accent-foreground"
              disabled={featuredListings.length === 0}
            >
              <Star className="h-4 w-4" />
              <span className="hidden sm:inline">Vedettes</span>
            ...
            <TabsTrigger 
              value="urgent" 
              className="gap-2 data-[state=active]:bg-gradient-urgent data-[state=active]:text-destructive-foreground"
              disabled={urgentListings.length === 0}
            >
            ...
            <TabsTrigger 
              ...
              disabled={boostedListings.length === 0}
            >
              <ArrowUp className="h-4 w-4" />
              <span className="hidden sm:inline">Remontées</span>
              <span className="bg-background/20 px-2 py-0.5 rounded-full text-xs">
          ...
          <TabsContent value="vedette" className="mt-0">
            {featuredListings.length > 0 ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                Aucune annonce vedette pour le moment
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="remontee" className="mt-0">
            {boostedListings.length > 0 ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                Aucune annonce remontée pour le moment
              </div>
            )}
          </TabsContent>
        </Tabs>
